...
class TestTranscription:
    ...
    async def test_transcribe_short_audio(self, short_audio_file, azure_transcription_backend):
        """Basic transcription returns text with timestamps."""
        result = await azure_transcription_backend.transcribe(short_audio_file)
    ...
    async def test_transcribe_returns_diarized_output(
    ...
    async def test_transcribe_multi_speaker_audio(
        self, multi_speaker_audio, azure_transcription_backend
    ):
        """Multiple speakers are identified in multi-speaker audio."""
        result = await azure_transcription_backend.transcribe(multi_speaker_audio)
    ...
    async def test_transcribe_with_time_offset(self, short_audio_file, azure_transcription_backend):
        """Time offset is correctly applied to timestamps."""
        offset = 900  # 15 minutes in seconds

        result = await azure_transcription_backend.transcribe(short_audio_file, time_offset=offset)
    ...
    async def test_transcribe_returns_readable_text(
        self, short_audio_file, azure_transcription_backend
    ):
