...
def get_config(require_text_api=False):
...
def validate_audio_file(file_path):
    ...
    if file_ext in api_supported:
        ...
        log(f"Attempting to convert anyway using ffmpeg...")
...
def convert_to_supported_format(file_path):
    """Convert audio/video file to M4A format if needed."""
    file_ext = Path(file_path).suffix.lower()
    ...
    try:
        # Use AAC codec for audio, extract audio from video files
        # -vn: no video (extract audio only)
        # -acodec aac: encode audio as AAC
        # -b:a 128k: set audio bitrate to 128kbps (good quality, reasonable size)
        # -ar 16000: sample rate 16kHz (optimal for speech recognition)
        subprocess.run(
            ['ffmpeg', '-i', file_path, '-vn', '-acodec', 'aac', 
             '-b:a', '128k', '-ar', '16000', '-y', temp_path],
            capture_output=True,
            check=True
        )
        ...
        log(f"ffmpeg error: {e.stderr.decode() if e.stderr else 'Unknown error'}")
...
def split_audio_file(file_path, chunk_duration=900):
    """Split audio file into chunks of specified duration (default 15 minutes)."""
    log(f"Splitting audio into {chunk_duration//60}-minute chunks using ffmpeg...")
    ...
    try:
        subprocess.run(
            ['ffmpeg', '-i', file_path, '-f', 'segment', '-segment_time', str(chunk_duration),
        ...
        log("Error: Could not split file. Make sure ffmpeg is installed.")
        sys.exit(1)


def transcribe_audio(audio_file_path, api_key, api_url, time_offset=0):
    ...
    try:
        with open(audio_file_path, 'rb') as audio_file:
            ...
            log(f"Sending to API for transcription...")
...
def correct_with_glossary(transcript, glossary_text, config, max_retries=3):
...
def process_chunk(chunk_info, config, glossary_text=None):
...
def write_output(text, output_file_path):
...
Note: Files longer than 25 minutes will be automatically split into chunks.
    ...
    parser.add_argument(
        ...
        nargs='?',
        default=None,
        help='Path to the output text file (optional - defaults to input filename with .txt extension)'
    )
    
    parser.add_argument(
        '--glossary', '-g',
        help='Path to a glossary file containing terms, names, acronyms, etc. for transcript correction'
    )
    ...
    try:
        ...
        if duration and duration > max_duration:
            log(f"Audio duration: {duration/60:.1f} minutes (exceeds {max_duration/60:.1f} min limit)")
            chunks, temp_dir = split_audio_file(converted_file, chunk_duration=900)  # 15-minute chunks
            ...
            transcription = transcribe_audio(converted_file, config['transcribe_key'], config['transcribe_url'])
