...
async def transcribe(
    ...
    *,
    ...
    if glossary:
        ...
        validate_input_size(glossary_text, settings.max_glossary_size, name="glossary file")
    ...
    try:
        ...
        if duration and duration > settings.max_duration_before_split:
            ...
            try:
                chunk_infos = [
                    (i, chunk, i * settings.chunk_duration) for i, chunk in enumerate(chunks_list)
                ]
...
def _open_converted_audio(file_path: str) -> tuple[str, str | None]:
    ...
    converted_path = convert_to_m4a(file_path)
