...
4. **Present Plan**: Share the plan synopsis in chat, highlighting the approach and implementation options.

5. **Write Plan File**: Write the plan to `plans/<task-name>-plan.md` and proceed to implementation.
...
### 2C. Commit Changes
1. **Write Phase Completion File**: Create `plans/<task-name>-phase-<N>-complete.md` following <phase_complete_style_guide>.

2. **Create Git Commit**: Automatically commit the changes using git with a commit message following <git_commit_style_guide>. Include:
   - Phase number and objective
   - Files/functions created/changed
   - Review status (approved/issues addressed)

3. **Report Progress**: Share a brief summary of phase completion and proceed to next phase automatically.
...
This project uses Conventional Commits for automated semantic versioning. Follow this format EXACTLY:
...
- `feat!:` or `fix!:` → major bump (X.0.0) - Breaking changes
- `chore:`, `docs:`, `test:`, `refactor:`, `style:` → no bump

**Breaking changes:** Add `!` after type: `feat!: change output format`

**Examples:**
- `fix: handle empty audio files gracefully`
- `feat: add speaker identification`
- `feat!: change JSON output schema`
...
The workflow runs fully autonomously from start to finish:
1. Planning phase completes and proceeds directly to implementation
2. Each phase automatically commits after successful review
3. Plan completion document is created and final summary is provided

The conductor works continuously until the entire plan is complete. Only consult the user if there are critical blocking issues that cannot be resolved autonomously.
</autonomous_execution>
